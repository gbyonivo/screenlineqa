...
context('PASSWORD_RESET user login', async () => {
  it('admin login', async () => {
    const overseer = await createTeacher({ role: ROLES.OVERSEER });
    const school = await createSchool({ overseer });
    const admin = await createTeacher({ role: ROLES.ADMIN, schoolId: school.id, status: 'PASSWORD_RESET' });
  ...
  it('teacher login', async () => {
    const overseer = await createTeacher({ role: ROLES.OVERSEER });
    const school = await createSchool({ overseer });
    const teacher = await createTeacher({ role: ROLES.TEACHER, schoolId: school.id, status: 'PASSWORD_RESET' });
  ...
  it('subject admin login', async () => {
    const overseer = await createTeacher({ role: ROLES.OVERSEER });
    const school = await createSchool({ overseer });
    const subjectAdmin = await createTeacher({ role: ROLES.SUBJECT_ADMIN, schoolId: school.id, status: 'PASSWORD_RESET' });
  ...
  it('parent login', async () => {
    const overseer = await createTeacher({ role: ROLES.OVERSEER });
    const school = await createSchool({ overseer });
    const parent = await createParent({ schoolId: school.id, status: 'PASSWORD_RESET' });
  ...
  it('student login', async () => {
    ...
    const student = await createStudent({ schoolId: school.id, classId: clazz.id, status: 'PASSWORD_RESET' });
